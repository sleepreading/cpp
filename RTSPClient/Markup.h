...
public:
	...
	// ×Ô¶¨Òå:
	private: void x_ElemToMap( std::map< std::string, std::string > & _map, std::string & sPath )
	{
		_map[ sPath ] = this->GetData();
		int i = 0;
		std::string sAttribName = this->GetAttribName(0);
		while ( sAttribName != "" )
		{
			char * s = (char*)_alloca( sAttribName.size() + 1 );
			strcpy( s, sAttribName.c_str() );
			s = strlwr( s );
			_map[ sPath + ":" + s ] = this->GetAttrib( sAttribName.c_str() );
			sAttribName = this->GetAttribName( ++i );
		}
		if ( this->IntoElem() )
		{
			while ( this->FindElem() )
			{
				char * s = (char*)_alloca( this->GetTagName().size() + 1 );
				strcpy( s, this->GetTagName().c_str() );
				s = strlwr( s );
				std::string sTempPath = sPath + "/" + s;
				this->x_ElemToMap( _map, sTempPath );
			}
			this->OutOfElem();
		}
	}
	public: bool XmlToMap( std::map< std::string, std::string > &_map )
	{
		this->ResetPos();
		if ( !this->FindElem() )
			return false;
		std::string sPath = this->GetTagName();
		char * s = (char*)_alloca( sPath.size() + 1 );
		strcpy( s, sPath.c_str() );
		s = strlwr( s );
		sPath = s;
		this->x_ElemToMap( _map, sPath );
		return true;
	}

	private: void x_ElemToMapShort( std::map< std::string, std::string > & _map, std::string & sPath )
	{
		_map[ this->GetTagName() ] = this->GetData();
		int i = 0;
		std::string sAttribName = this->GetAttribName(0);
		while ( sAttribName != "" )
		{
			...
			strcpy( s, sAttribName.c_str() );
			s = strlwr( s );
			_map[ s ] = this->GetAttrib( sAttribName.c_str() );
			sAttribName = this->GetAttribName( ++i );
		}
		if ( this->IntoElem() )
		{
			while ( this->FindElem() )
			{
				...
				s = strlwr( s );
				std::string sTempPath = sPath + "/" + s;
				this->x_ElemToMapShort( _map, sTempPath );
			}
			this->OutOfElem();
		}
	}
	public: bool XmlToMapShort( std::map< std::string, std::string > &_map )
	{
		this->ResetPos();
		if ( !this->FindElem() )
			return false;
		...
		s = strlwr( s );
		sPath = s;
		_map.insert( std::make_pair( this->GetTagName(), this->GetData() ) );
		this->x_ElemToMapShort( _map, sPath );
		return true;
	}
